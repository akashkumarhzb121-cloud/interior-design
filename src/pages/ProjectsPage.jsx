...
// ── Media item from backend: { url, publicId, resourceType }
// Resolves all possible shapes into a clean array
function resolveMedia(service) {
  // Prefer the full media[] array (new uploads)
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        // Backend wraps array as { success, data: [...] }
...
function ServiceCard({ service, index }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-6">
        {/* Icon row — shown only when media is present (replaces banner above) */}
        {hasMedia && (

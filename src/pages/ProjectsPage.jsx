  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            <div className="hidden lg:flex items-center gap-2">
              {categories.map((category) => (
                <button key={category} onClick={() => handleCategoryChange(category)} className={cn('px-4 py-2 rounded-full text-sm font-medium transition-colors', activeCategory === category ? 'bg-gold text-charcoal' : 'bg-secondary text-foreground hover:bg-gold/20')}>{category}</button>
              ))}
            </div>
            <button onClick={() => setShowFilters(!showFilters)} className="lg:hidden flex items-center gap-2 px-4 py-2 bg-secondary rounded-lg"><Filter className="w-4 h-4" />Filters</button>
          ...
          {showFilters && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="lg:hidden mt-4 flex flex-wrap gap-2">
              {categories.map((category) => (
                <button key={category} onClick={() => { handleCategoryChange(category); setShowFilters(false); }} className={cn('px-4 py-2 rounded-full text-sm font-medium transition-colors', activeCategory === category ? 'bg-gold text-charcoal' : 'bg-secondary text-foreground hover:bg-gold/20')}>{category}</button>
              ))}
            </motion.div>
          )}
        </div>
      ...
      <Section className="bg-background pt-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8"><p className="text-muted-foreground">Showing <span className="text-foreground font-medium">{filteredProjects.length}</span> projects{activeCategory !== 'All' && <> in <span className="text-gold">{activeCategory}</span></>}</p></div>
          {loading ? (
            ...
            <div className="text-center py-16">
              <div className="w-20 h-20 mx-auto bg-secondary rounded-full flex items-center justify-center mb-4"><Search className="w-8 h-8 text-muted-foreground" /></div>
              <h3 className="text-xl font-semibold">No projects found</h3>
              <p className="text-muted-foreground mt-2">Try adjusting your search or filter criteria</p>
              <Button variant="outline" className="mt-4" onClick={() => { setSearchQuery(''); handleCategoryChange('All'); }}>Clear Filters</Button>
            </div>
          )}
        </div>
      </Section>
      ...
      <Section className="bg-sand">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <Link to="/booking"><Button variant="gold" size="xl">Book Consultation<ArrowRight className="w-5 h-5" /></Button></Link>
        </div>
      </Section>
    </>
  )
      ...
      <Link to={projectId ? `/projects/${projectId}` : '/projects'}>
        <div className="relative aspect-[4/3] rounded-xl overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6">
            <span className="inline-block px-3 py-1 bg-gold/90 text-charcoal text-xs font-medium rounded-full mb-2">{project.category}</span>
            <h3 className="text-xl font-serif font-semibold text-white group-hover:text-gold transition-colors">{project.title}</h3>
            <p className="mt-1 text-white/80 text-sm flex items-center gap-1"><MapPin className="w-3 h-3" />{project.location}</p>
          </div>
          <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
            <div className="w-10 h-10 bg-gold rounded-full flex items-center justify-center"><ArrowRight className="w-5 h-5 text-charcoal" /></div>
          </div>
        </div>
      </Link>
    </motion.div>
